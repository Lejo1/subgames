...
subgames.register_on_blast(function(pos, intensity, lobby)
  if lobby == "mesewars" then
    ...
    if not plobby or plobby == 0 or mesewars.lobbys[plobby].ingame ~= true or not (nodename == "default:sandstone" or nodename == "default:obsidian" or nodename == "default:glass" or nodename == "default:steelblock" or nodename == "default:chest") then
      return false
    else local spos = minetest.pos_to_string(pos)
      if not mesewars.lobbys[plobby].mapblocks[spos] then
        mesewars.lobbys[plobby].mapblocks[spos] = node
      end
    end
  end
end)
